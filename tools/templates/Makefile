...
build: cmd/*/*.go internal/*/*.go pkg/*/*.go scripts/version.sh Makefile vendor
	@echo "compile"
	@go build -ldflags "-X 'main.AppVersion=`sh scripts/version.sh`'" cmd/xxxxx/main.go && \
	mkdir -p build/xxxxx/bin && mv main build/xxxxx/bin/xxxxx && \
	mkdir -p build/xxxxx/configs && cp configs/xxxxx/* build/xxxxx/configs
